    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        amb.launchCoroutinesBt.setOnClickListener {
            ...
            GlobalScope.launch(Dispatchers.Default) {
                Log.v(getString(R.string.app_name), "Top coroutine thread: ${Thread.currentThread().name}, Job: ${coroutineContext[Job]}")
                val upper = sleep("Upper",random.nextLong(SLEEP_LIMIT))
                withContext(Dispatchers.Main + Job()){
                    Log.v(getString(R.string.app_name), "But this code is executing in thread: ${Thread.currentThread().name}, Job: ${coroutineContext[Job]}")
                    amb.upperTv.text = upper
                }

                Log.v(getString(R.string.app_name), "Top coroutine thread after main: ${Thread.currentThread().name}, Job: ${coroutineContext[Job]}")

               launch(Dispatchers.IO){
                    Log.v(getString(R.string.app_name), "Lower coroutine thread: ${Thread.currentThread().name}, Job: ${coroutineContext[Job]}")
